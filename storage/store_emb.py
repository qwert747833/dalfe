...
def parse_args():
    ...
    parser.add_argument("--device", type=str, default="cuda", help="Device to use for computation (default: cuda:1)")
...
def save_embeddings(args):
    ...
    print("args.device =", args.device)  # 确认是否为 "cuda"
    ...
    print(f"[Debug] 训练集长度: {len(train_set)}")
    print(f"[Debug] 检验集长度: {len(test_set)}")
    print(f"[Debug] 测试集长度: {len(val_set)}")
    print(f"[Debug] 当前运行模式: {args.divide}") 
    ...
    for i, (x, y, x_mark, y_mark) in enumerate(data_loader):
        if i % 1000 == 1:
            print(f"[Debug] 批次 {i} 数据形状: x={x.shape}, x_mark={x_mark.shape}")
